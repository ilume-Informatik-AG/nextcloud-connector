# Nextcloud Outbound Connector

A Camunda 8 outbound (function) connector enabling seamless file and share management in
**Nextcloud** directly from your BPMN processes. The connector uses the Nextcloud WebDAV and OCS
APIs for secure, authenticated file and share operations.
...
See the root README's [Authentication & Secrets](../README.md#authentication--secrets) section for
how `secrets.<NAME>` resolves at runtime, and the App Password recommendation.
...
* **Inputs**:
    ...
    * **Allow Public Upload** (`action.publicUpload`): (**legacy attribute**. Don't use for File upload request!) Boolean, allows uploads to public shared folders (only for folders with `PUBLIC_LINK`).
